...
export default function Page() {
  ...
  const addRows = useCallback(async () => {
    const ans = window.prompt('몇 행을 추가할까요?', '10');
    if (ans == null) return;
    const count = Math.trunc(Number(ans) || 0);
    if (count < 1) return;
  ...
  return (
    <>
      <div className="nav">
        ...
        <button className="btn" style={{ background: '#ebf8ff', borderColor: '#90cdf4', fontWeight: 'bold' }} onClick={addRows}>＋ 행 추가</button>
