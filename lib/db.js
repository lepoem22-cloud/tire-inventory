...
export async function ensureSchema() {
  ...
  `;
  // 기존 backup_log에 note 컬럼이 없으면 추가 (마이그레이션)
  await sql`ALTER TABLE backup_log ADD COLUMN IF NOT EXISTS note TEXT`;

  // K·L 등 출고 채널 메모 이력 (8시 초기화 전까지 쌓이는 미정산 메모)
  await sql`
    CREATE TABLE IF NOT EXISTS ship_memo (
      id        SERIAL PRIMARY KEY,
      ts        TIMESTAMPTZ NOT NULL DEFAULT now(),
      user_id   TEXT NOT NULL DEFAULT '',
      tire_pk   INTEGER NOT NULL,
      channel   TEXT NOT NULL DEFAULT '',
      memo      TEXT NOT NULL DEFAULT ''
    )
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_ship_memo_tire ON ship_memo (tire_pk)`;
  await sql`
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_edit_log_id ON edit_log (id DESC)`;
  _ready = true;
}
