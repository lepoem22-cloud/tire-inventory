...
export async function GET() {
  try {
    ...
    const { rows } = await sql`
      SELECT id, ymd, hm, brand, pattern, pcode, size, pr, dot,
...
export async function DELETE(req) {
  try {
    ...
    if (body.all === true) {
      await sql`TRUNCATE TABLE backup_log RESTART IDENTITY`;
      return NextResponse.json({ ok: true, deleted: 'all' });
    }
    const ids = Array.isArray(body.ids) ? body.ids.map(Number).filter(Boolean) : [];
    if (!ids.length) return NextResponse.json({ ok: false, msg: '삭제할 항목이 없습니다' }, { status: 400 });
    const r = await sql.query(`DELETE FROM backup_log WHERE id = ANY($1::int[])`, [ids]);
